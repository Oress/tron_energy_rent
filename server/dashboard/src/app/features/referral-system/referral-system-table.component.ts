...
import { ReferralSystemRow, ReferralSystemService } from '../../core/api';
...
@Component({
  ...
  imports: [RequestStateComponent, TablePaginatorComponent, TrxAmountPipe],
  templateUrl: './referral-system-table.html',
})
export class ReferralSystemTableComponent {
  private readonly referralSystemApi = inject(ReferralSystemService);
  private readonly filterService = inject(DashboardFilterService);
  private readonly destroyRef = inject(DestroyRef);
  private requestSequence = 0;

  readonly rows = signal<ReferralSystemRow[]>([]);

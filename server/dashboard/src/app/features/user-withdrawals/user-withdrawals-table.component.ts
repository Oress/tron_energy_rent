...
import { UserWithdrawalRow, UserWithdrawalsService } from '../../core/api';
...
@Component({
  selector: 'app-user-withdrawals-table',
  imports: [DatePipe, RequestStateComponent, TablePaginatorComponent, TrxAmountPipe],
  templateUrl: './user-withdrawals-table.html',
})
export class UserWithdrawalsTableComponent {
  private readonly withdrawalsApi = inject(UserWithdrawalsService);
  private readonly filterService = inject(DashboardFilterService);
  private readonly destroyRef = inject(DestroyRef);
  private requestSequence = 0;

  readonly rows = signal<UserWithdrawalRow[]>([]);
